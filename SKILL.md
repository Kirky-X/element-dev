...
- **kb**（本地知识库）— 本地 Qdrant 知识库，2 类 sidebar 文档分类存储（design-guide 17 篇 + component 82 篇 = 99 文档），向量嵌入（默认 `paraphrase-MiniLM-L3-v2`，ModelScope/云端可切换）+ BM25 关键词索引 + 可选 FlashRank 重排。子动作：query/build/reindex/merge/update-description/update-links/link-auto/migrate-embed-model/config。解决"**本地能查什么**"。
- **fetch**（在线抓取）— 直接 HTTP GET 抓取 element-plus.org 静态文档页，提取 `<main>` 内容并转 Markdown。作为 kb description/links 填充的合法通道。解决"**网上有什么**"。
- **config**（配置）— 显示/修改 embed_model、rerank_model、db_path 等配置项。解决"**怎么切换**"。
...
| `unknown payload field` | set_payload 写入白名单外字段 | 检查字段名，仅 10 字段允许 |
